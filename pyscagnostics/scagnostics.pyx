...
def scagnostics(*args, bins=50, remove_outliers=True):
    ...
    cdef double[:] c_x = x
    cdef double[:] c_y = y
    cdef int[:] c_length = np.array([x.shape[0]], dtype=np.int32).astype(np.int32)
    cdef int[:] c_bins = np.array([bins], dtype=np.int32).astype(np.int32)
    cdef int[:] c_outlierRmv = np.array([int(remove_outliers)], dtype=np.int32).astype(np.int32)
